...
NSString *BNRDocPath() {
  ...
  NSString *filename = [pathList[0] stringByAppendingPathComponent:@"data.td"];
  NSLog(@"plist data path is: %@", filename);
  return filename;
}
...
- (BOOL)application:(UIApplication *)application
  ...
  CGRect tableFrame =
      CGRectMake(0, 80, winFrame.size.width, winFrame.size.height - 100);
  ...
  // Tell the table view which class to instantiate whenever it needs to create
  // a new cell
...
- (NSInteger)tableView:(UITableView *)tableView
  ...
  // equal to the number of items in the task array
  return [self.tasks count];
}

- (UITableViewCell *)tableView:(UITableView *)tableView
         cellForRowAtIndexPath:(NSIndexPath *)indexPath {
  // To improve performance, this method first checks for an existing cell
  // object that we can reuse
  // If there is not one, then a new cell is created
  UITableViewCell *c =
      [self.taskTable dequeueReusableCellWithIdentifier:@"Cell"];

  // Then we (re)configure the cell based on the model object, in this case the
  // task array
